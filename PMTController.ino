...
3. Monitor tube trip status (Hamamatsu H7422P-40 with M9012 controller), and provide
  ...
  Tube status is provided by a signal line derived from an LM339 comparator in the
...
const float controller_version  = 0.1;  // a number for the version of this controller code.

// assign hardware ports
const unsigned PMT_monitor[] = {1, 0};   // ADCs to read PMT current
// const unsigned PMT_cmd[]  = {DAC0, DAC1}; // note, not using dacs...
...
const char PMTId[][10] = {'H7422P-40\0', 'H10721020\0'};
  ...
  switch (cmd) {
    ...
    case 'c':  // return the average (or peak) pmt current on the specified PMT
      ...
      if (device >= 0 && device < NPMT) {  // note the return value indicates 'p' or 'm' depending on mode
        sprintf(str, "%s%1d %8.3f uA", mode, device, lastMeas[device]);
        SerialUSB.println(str);
      }
      break;
    ...
    case 's':  // get overall status for all listed PMTs
      for (device = 0; device < NPMT; device++) {
        sprintf(str, "PMT%1d: %s", device, PMTId);  // ID
        SerialUSB.print(str);
        SerialUSB.print("; ");
        value = analogRead(PMT_monitor[device]) * ADCscale * GI_PMT[device]; // current
        sprintf(str, "I%1d %6.3f uA", device, value);
        SerialUSB.print(str);
        SerialUSB.print("; ");
        sprintf(str, "%s%1d %8.3f uA", mode, device, lastMeas[device]);  // peak or mean current (and mode)
        SerialUSB.print(str);
        SerialUSB.print("; ");
        sprintf(str, "cmd V %1d %5.3f V", device, CMD_PMT[device]);  // command voltage
        SerialUSB.print(str);
        SerialUSB.print("; ");
...
void resetPMT(int pmt) {
  ...
  delayMicroseconds(10000);  // just a brief pulse, 10 usec or so.
